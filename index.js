    ...
    function closeExtensionsUi() {
        tryClickVisible(
            doc.querySelector('#extensionsMenu')
                ?.closest?.('.drawer-content, .popup, .menu_button_popup, .draggable, [role="menu"]')
                ?.querySelector?.('.close-button, .drawer-close, .menu_button_close, [data-action="close"], [aria-label="Close"], [title="Close"]')
        );

        tryClickVisible(
            doc.querySelector('.drawer-overlay, .popup-overlay, .modal-backdrop, .backdrop, .ui-widget-overlay')
        );
