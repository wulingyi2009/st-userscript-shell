...
function createShell(hostWin) {
    ...
    const IDS = {
        ...
        header: 'stus-shell-header',
        actions: 'stus-shell-actions',
    ...
    const api = {
        ...
        async destroy() {
            ...
            const menuItem = doc.getElementById(IDS.menuItem);
            const panel = doc.getElementById(IDS.panel);
            const backdrop = doc.getElementById(IDS.backdrop);
            const style = doc.getElementById(IDS.style);
            ...
            try { delete hostWin.STUserScriptShell; } catch {}
            try { delete window.STUserScriptShell; } catch {}
        },
    };
...
@media (max-width: 900px) {
    ...
    function ensurePanel() {
        ...
        if (!panel) {
            ...
            panel.innerHTML = `
<header id="${IDS.header}" class="stus-shell-header">
    ...
    <button id="${IDS.close}" type="button" aria-label="关闭">×</button>
</header>
<div id="${IDS.body}"></div>
            `;
        ...
        if (bodyEl && !bodyEl.childNodes.length && !state.renderCleanup) {
...
function ensureMenuItem() {
    ...
    const nextHtml = `
<span class="extensionsMenuExtensionButton">${state.iconHtml}</span>
<span>${escapeHtml(state.title)}</span>
    ...
    if ((item.innerHTML || '').trim() !== nextHtml) {
        item.innerHTML = nextHtml;
    }
...
function installObserver() {
    ...
    // 不再监听整个 body 的所有变化，避免 observer -> innerHTML -> observer 自激死循环
    const timer = setInterval(() => {
        try {
            ensureMenuItem();
        } catch {}
...
<div class="stus-empty">
    <strong>壳已就绪</strong>
    <div>这里是一个可复用的 SillyTavern userscript 空壳。</div>
    <div>下一步请在你的业务脚本里调用 <code>window.STUserScriptShell.mount(renderFn)</code>，把真正的插件 UI 挂进来。</div>
</div>
        `;
    }

    function getPanelElement() {
        return doc.getElementById(IDS.panel) || ensurePanel();
    }

    function getHeaderElement() {
        return doc.getElementById(IDS.header) || ensurePanel()?.querySelector?.(`#${IDS.header}`) || null;
    }
...
<div class="stus-empty">
    ...
    async function unmount(options = {}) {
        ...
        const body = doc.getElementById(IDS.body) || getMountBody();
        if (!body) return;
    ...
    const toggleBtn = doc.querySelector(
        '#extensionsMenuButton, [aria-controls="extensionsMenu"], [data-target="extensionsMenu"], [data-menu="extensionsMenu"], .extensionsMenuButton'
    );

    // 1) 优先尝试点击真正的扩展菜单按钮（通常是 toggle）
    if (tryClickVisible(toggleBtn)) {
        return true;
    }

    // 2) 如果菜单在某个 drawer/popup 里，尝试点它自己的关闭按钮
    const closeBtn = menuRoot?.closest?.('.drawer-content, .popup, .menu_button_popup, .draggable, [role="menu"]')
        ?.querySelector?.('.close-button, .drawer-close, .menu_button_close, [data-action="close"], [aria-label="Close"], [title="Close"]');

    if (tryClickVisible(closeBtn)) {
        return true;
    }

    // 3) 最后再发一次 Escape 作为兜底
    dispatchEscape(doc);
    ...
    function dispatchEscape(target) {
        ...
        try { target.dispatchEvent(new KeyboardEvent('keydown', evtInit)); } catch {}
        try { target.dispatchEvent(new KeyboardEvent('keyup', evtInit)); } catch {}
    }
